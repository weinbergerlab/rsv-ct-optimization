% TODO: Better calendrical cues in plots, maybe also with bars?
% TODO: establish need in intro
% TODO: adverse effects of immunoprophylaxis
...
\includegraphics[height=2.5in]{EPH}
\hspace{40pt}
\includegraphics[height=2.5in]{Medicine}
...
Our goal was to construct a model of annual RSV incidence that allows us to easily estimate complex characteristics of RSV seasons, such as season onset and offset, or preventable RSV case fraction.
...
<<seasonByRegion, results='hide'>>=
    ...
    ggplot(data) +
        ...
        scale_y_continuous(minor_breaks=seq(1, 52), breaks=epiWeekBreaks, labels=epiWeekLabels) + 
        labs(y=NULL) + 
        theme(
...
Statewide, our model shows that \Sexpr{regionStrategyCoverage("all", "aap", 0)} of RSV cases (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}) occur while palivizumab, administered according to the AAP guidelines, offers protection.
...
\caption{Fraction of RSV cases occurring in Connecticut during the protection window of palivizumab prophylaxis administered according to the AAP guidelines, by region}
...
Compared to our regional analysis, the three regimens based on statewide recent-years analysis produced non-superior results in all regions except for the low-RSV counties, where they produced inferior results. (Figure \ref{fig:coverageByRegionRegimenWithSliding}.)
...
% TODO bring back discussion of cost from draft 1?
