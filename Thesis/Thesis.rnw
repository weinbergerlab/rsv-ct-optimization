% TODO: DOI in bib?
% TODO: Prettier abstract sectioning
% TODO: Better calendrical cues in plots
...
\newcommand{\abstractsection}[1]{\makebox[1in][l]{\textbf{#1}}}

\renewcommand*{\thefootnote}{\fnsymbol{footnote}}

\title{Optimization of respiratory syncytial virus prophylaxis using a generalized additive model of Connecticut hospitalizations}
\def\headertitle{Optimization of RSV prophylaxis using a generalized additive model of Connecticut hospitalizations}
...
\abstractsection{Methods} We describe a generalized additive model of RSV incidence using cubic cyclic penalized splines and apply that model to hospital admissions records from Connecticut between \Sexpr{formatDate(startDate)} and \Sexpr{formatDate(endDate)} to estimate fraction of RSV cases in Connecticut preventable by immunoprophylaxis administered according to the AAP guidelines. We also formulate several alternate immunoprophylaxis regimens with the same net pharmaceutical cost, estimate the fraction preventable by those alternate regimens.

\abstractsection{Results} At 95\% confidence level, we found alternate regimens that account for spatial variability of RSV incidence at the county level to have superior coverage (to that of the AAP-recommended regimen) in some regions of Connecticut, and non-inferior coverage in other regions. We also found alternate regimens that account for temporal RSV variability at the annual level to have non-superior coverage to our spatially-adjusted regimens. Overall, we found our best-performing spatially-adjusted alternate to AAP guidelines to provide a XX\% statewide increase in RSV prophylaxis coverage over the AAP regimen.

\abstractsection{Conclusion} We conclude that the increased prophylaxis coverage provided by regimens based on our county-level spatial analysis warrants further analysis to determine whether the potential reduction in hospitalization rate is nullified by the potential increase in implementation cost. Our temporal analysis at the annual level showed no additional benefit beyond the spatial analysis, while simultaneously increasing implementation complexity. Overall, we recommend county-level spatial analysis of RSV incidence as the starting point for RSV immunoprophylaxis optimization in Connecticut. 
...
In children, however, the infection often incites more serious respiratory illness, making RSV a leading cause of hospitalization among infants.\cite{Hall:2009gi,Hall:2013jo,Zhou:2012ho,Boyce:2000gz}
...
Palivizumab has been found to be effective in reducing hospitalizations due to RSV; in double-blinded trials in high-risk infants, it reduced hospitalization rate by 45-55\% compared to placebo.\cite{Group:1998ih,Feltes:2003wm}
...
The Department of Public Health of the state of Connecticut, via its Office of Health Care Access (OHCA), maintains a database of all hospitalizations in every non-federal short-stay acute-care general hospital in the state. Previous analysis of this database has shown regional variability in time of peak RSV incidence within Connecticut.\cite{Noveroske:y4fi3188}
...
\begin{itemize}
	...
	\item Can we use spatial analysis of RSV to propose alternate regimens, either statewide or regional, that would improve effectiveness over AAP-recommended prophylaxis, without increasing treatment cost?
...
Our goal was to construct a model of RSV incidence over time that allows us to easily estimate complex characteristics of RSV seasons, such as season onset and offset, or preventable RSV case fraction.
...
We were willing to assume that RSV incidence can be modeled as a twice-differentiable piecewise Poisson time series.
...
We developed a custom R package for estimation of arbitrary characteristics of infectious disease outbreaks (as long as the outbreaks are amenable to GAM); this package is available at \href{https://github.com/airbornemint/outbreakpredict}{\nolinkurl{github.com/airbornemint/outbreakpredict}}.
...
\begin{itemize}
    ...
    \item One discontiguous region comprised of all remaining counties in Connecticut: \Sexpr{gsub("(.*), (.*)", "\\1, and \\2", paste(lowIncidenceCounties, collapse=", "))}
\end{itemize}
...
We chose to aggregate low-incidence counties into a single region because our analysis in those counties individually would have yielded results with unacceptably low statistical power.
...
\subsection{Devising alternate all-years prophylaxis regimens}
...
\subsection{Evaluating alternate prophylaxis regimens}

We evaluated each of those alternate regimens in the same way we evaluated the current AAP guidelines, as described above. 

For the three alternate regimens based on statewide data, we estimated preventable fraction statewide, as well as in each region.

For the three alternate regimens based on regional data, we estimated preventable fraction in the corresponding region.
...
\begin{itemize}
    ...
    \item \textbf{Biweekly rounding}: prophylaxis start and end date rounded to the nearest two-week calendar period, with two-week period boundary chosen , and % TODO: specifics
    \item \textbf{Monthly rounding}: prophylaxis start and end date rounded to the nearest four-week calendar period.  % TODO: specifics and also not actually in code now
\end{itemize}
...
\subsection{Estimating characteristics of RSV surseasons}
...
\subsection{Devising alternate recent-years prophylaxis regimens}

Finally, to assess the impact of any long-term trends in RSV season timing on prophylaxis, we formulated three additional prophylaxis regimens. Here, each prophylaxis regimen is adjusted annually based on RSV data from the preceding surseason:
...
As before, these alternate regimens involved six monthly doses of palivizumab giving 24 weeks of protection. 
...
\caption{Weekly RSV relative cumulative incidence in Connecticut, by region}
\label{fig:coverageByRegionAAPInsight}
\end{figure*}
\setcounter{figure}{0}

\paragraph{RSV season in Fairfield county begins early and ends late}
...
\paragraph{RSV season in low-incidence counties begins late}

Meanwhile, in comparison with the statewide RSV season onset and offset, the low-incidence region has RSV season with later onset at \Sexpr{regionOnset("lowIncidence")} weeks (\Sexpr{regionOnsetCI("all", level)} weeks); season offset shows no change from the statewide median. (Figure \ref{fig:seasonByRegion}.)
...
\paragraph{AAP guidelines perform best in low-incidence counties and worst in Fairfield county}

Statewide, our model shows that \Sexpr{regionStrategyCoverage("all", "aap", 0)} of RSV cases (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}) occur while palivizumab, administered according to AAP guidelines, offers protection.

The AAP guidelines perform better in the low-incidence counties, where \Sexpr{regionStrategyCoverage("lowIncidence", "aap", 0)} cases (\Sexpr{regionStrategyCoverageCI("lowIncidence", "aap", 0, level)}) occur during the prophylaxis time window, but worse in Fairfield county, where \Sexpr{regionStrategyCoverage("Fairfield", "aap", 0)} cases (\Sexpr{regionStrategyCoverageCI("Fairfield", "aap", 0, level)}) occur during this time. (Figure \ref{fig:coverageByRegionAAP}.)
...
The cause of this is evident in figure \ref{fig:coverageByRegionAAPInsight}, showing temporal misalignment between the RSV season (delineated by median \Sexpr{seasonThreshold * 100}\%  onset and \Sexpr{(1 - seasonThreshold) * 100}\% offset in each region), and the time window of the AAP-recommended prophylaxis regimen. Fairfield county, owing to its early season onset, sees disproportionately many cases before prophylaxis administration begins; on the other hand, the low-incidence counties, owing to their late season onset and relatively shorter RSV seasons, see almost all of their RSV cases within the prophylaxis window.

\paragraph{Spatially-informed regimens are superior to AAP recommendations overall and in Hartford county}
...
Statewide, the AAP guidelines offer protection for \Sexpr{regionStrategyCoverage("all", "aap", 0)} cases (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}); meanwhile, the regimens based on all-years statewide onset produce the weakest improvement at \Sexpr{regionStrategyCoverage("all", "stateOnset", 0)} coverage (\Sexpr{regionStrategyCoverageCI("all", "stateOnset", 0, level)}), and other alternate regimens yield similar results.

In Hartford county, the improvement is similar. Here, the AAP guidelines protect \Sexpr{regionStrategyCoverage("Hartford", "aap", 0)} of cases (\Sexpr{regionStrategyCoverageCI("Hartford", "aap", 0, level)}); the regimens based on all-years regional offset generate the weakest improvement to \Sexpr{regionStrategyCoverage("Hartford", "countyOffset", 0)} protection (\Sexpr{regionStrategyCoverageCI("Hartford", "countyOnset", 0, level)}), and other alternate regimens are comparable.
...
<<coverageByRounding, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        theme_light(base_size=plotTextBaseSize) +
        scale_y_continuous(labels=latexPercent, limits=c(0.935, 0.965), breaks=seq(0, 1, by=.02), expand=c(0.05, 0)) +
        scale_color_manual(labels=stratLabels[c(seq(1, 6), 10)], values=c(rep(twoTone, each=3), grey(0))) + 
...
After we applied weekly, biweekly, and monthly rounding to start and end dates of our six alternate immunoprophylaxis regimens, overall we found that longer rounding intervals diminished the net benefit of spatial analysis.

With weekly rounding, our regimens were non-inferior to the AAP recommendation, and produced an increase in preventable fraction from AAP's \Sexpr{regionStrategyCoverage("all", "aap", 0)} (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}) to \Sexpr{bestCaseCoverage(1)} (\Sexpr{bestCaseCoverageCI(1, level)}) statewide. The increase was most notable in Fairfield and Hartford counties. % TODO Details?

Biweekly rounding yielded regimens similarly non-inferior to the AAP recommendation, with an increase in preventable fraction to \Sexpr{bestCaseCoverage(2)} (\Sexpr{bestCaseCoverageCI(2, level)}) statewide. The increase was most notable in Fairfield and New Haven counties. % TODO Details?
...
<<onsetOffsetByYear, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        theme_light(base_size=plotTextBaseSize) +
        geom_errorbar(aes(ymin=lower, ymax=upper, x=epiyear), width=.1) + 
        geom_point(aes(y=median, x=epiyear)) +
        facet_wrap(~variable, ncol=1, scales="free_y", labeller=onsetOffsetLabeller) +
...
\paragraph{Recent-years prophylaxis regimens are non-superior to others.}

Compared to our regional analysis, the three regimens based on statewide recent-years analysis produced non-superior results in all regions except for the low-incidence counties, where they produced inferior results. (Figure \ref{fig:coverageByRegionRegimenWithSliding}.)
...
Similarly, we assumed the protection by palivizumab is constant at 100\% throughout the protection period; however, not only is breakthrough RSV illness known to occur in patients on palivizumab, but it is an indication for discontinuing the remained of the prophylaxis regimen. 
...
Our analysis confirms regional variation of RSV season timing in Connecticut, defined by RSV season onset and offset, and observable at the county level. We have also shown a misalignment between the prophylaxis regimen recommended by the AAP and the timing of the RSV season in Fairfield County, resulting in the start of the RSV season occurring outside of the prophylaxis window.

Our attempt to optimize timing of the prophylaxis regimen (without changing it duration and therefore its cost) shows a theoretical possibility of a 1-2\% increase in preventable RSV cases each year, occurring primarily in counties with a higher RSV burden (Fairfield, Hartford, and New Haven counties).

However, that theoretical improvement rapidly wanes when faced with practicalities of clinical practice guideline implementation; in order to derive real-world benefit from our analysis, clinical practice guidelines would need to be localized at the county level, and the timing of prophylaxis adjusted by less than a whole month in each region. 

As a result, we conclude that the case for regional adjustment to RSV immunoprophylaxis guidelines is weak.

Although we found a weak statewide trend in RSV season onset becoming earlier over time, further analysis did not yield any improvements to prophylaxis based on that trend; the results of our temporal analysis were no stronger than the results of our spatial analysis. Since temporal variation in clinical guidelines would lead to a higher burden of implementation than spatial variation, we see no reason to base RSV immunoprophylaxis clinical practice guidelines on temporal variation in RSV reason timing.

% TODO proofread references
