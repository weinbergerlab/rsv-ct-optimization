...
%  * "Coverage" -> preventable fraction
%  * "Statewide"

<<options, echo=FALSE>>=
    opts_chunk$set(echo=FALSE, message=FALSE, root.dir=".texpadtmp")
@
...
Owing to the high cost, the American Academy of Pediatrics (AAP) recommends palivizumab immunoprophylaxis only for high-risk infants, and only during RSV season. These guidelines were most recently revised in 2014.
...
\begin{itemize}
	...
	\item Can we use spatial analysis of RSV to propose alternate regimens, either state-wide or regional, that would improve effectiveness over AAP-recommended prophylaxis, without increasing treatment cost?
...
Due to changes in diagnostic coding, we excluded data prior to July 1996; surveillance year 2014 had incomplete data and was also excluded.
...
For the three alternate regimens based on state-wide data, we estimated preventable fraction statewide, as well as in each region.

For the three alternate regimens based on regional data, we estimated coverage in the corresponding region.
...
<<seasonByRegion, results='hide'>>=
    data = thresholdsByCounty %>% 
        ...
        mutate(variable=factor(variable, levels=c("onset", "offset"))) %>%
        cast(county + variable ~ pctile)
        
    tikz("figure/seasonByRegion.tex", width=plotWidth, height=plotHeight, pointsize=10)
    ggplot(data) +
...
@
\begin{center}
\input{figure/seasonByRegion.tex}
...
<<coverageByRegionAAP, results='hide'>>=
    tikz("figure/coverageByRegionAAP.tex", width=plotWidth, height=plotHeight, pointsize=10)
    ...
    dev.off()
@
\input{figure/coverageByRegionAAP.tex}
...
The cause of this is evident in Figure \ref{fig:coverageByRegionAAPInsight}; temporal misalignment between the RSV season (delineated by median \Sexpr{seasonThreshold * 100}\%  onset and \Sexpr{(1 - seasonThreshold) * 100}\% offset in each region), and the time window of AAP prophylaxis regimen. Fairfield county, owing to its early season onset, has disproportionately many cases before prophylaxis administration begins; on the other hand, the low-incidence counties, owing to their late season onset, have their RSV seasons almost entirely contained within the prophylaxis window.

\begin{figure}
<<coverageByRegionAAPInsight, results='hide'>>=
    tikz("figure/coverageByRegionAAPInsight.tex", width=plotWidth, height=plotHeight*3/2, pointsize=10)
...
#            annotation_custom(linesGrob(gp=gpar(col=color)), xmin=(xmin + xmax) / 2, ymin=y-0.02, xmax=(xmin + xmax) / 2, ymax=y+0.02) + 
        ...
        plot = plot + 
            ...
            scale_y_continuous(labels=latexPercent, breaks=seq(0, 1, by=.2), limits=c(0, 1), expand=c(0.2, 0)) +
            labs(x=c) +
            theme(
    ...
    dev.off()
@
\input{figure/coverageByRegionAAPInsight.tex}
...
Based on all-years data, the six spatially adjusted prophylaxis regimens, when compared to the AAP guidelines, yield non-inferior results in all regions, and superior results both statewide and in Hartford county. (Figure \ref{fig:coverageByRegion}.)

Statewide, the AAP guidelines generate protection for \Sexpr{regionStrategyCoverage("all", "aap")} cases (\Sexpr{regionStrategyCoverageCI("all", "aap", level)}); meanwhile, the regimens based on all-years statewide onset produce the weakest improvement at \Sexpr{regionStrategyCoverage("all", "stateOnset")} coverage (\Sexpr{regionStrategyCoverageCI("all", "stateOnset", level)}), and other alternate regimens yield similar results.

In Hartford county, the improvement is more noteworthy. Here, the AAP guidelines protect \Sexpr{regionStrategyCoverage("Hartford", "aap")} of cases (\Sexpr{regionStrategyCoverageCI("Hartford", "aap", level)}); the regimens based on all-years regional offset generate the weakest improvement to \Sexpr{regionStrategyCoverage("Hartford", "countyOffset")} protection (\Sexpr{regionStrategyCoverageCI("Hartford", "countyOnset", level)}), and other alternate regimens are comparable.
...
<<coverageByRegion, results='hide', warning=FALSE>>=
    data = unprotectedByCounty %>% 
        filter(county %in% countiesForPlots)
    tikz("figure/coverageByRegionRegimen.tex", width=plotWidth, height=plotHeight, pointsize=10)
    ggplot(data) +
        theme_light(base_size=10) +
        scale_y_continuous(labels=latexPercent, limits=c(0.9, 1), breaks=seq(0, 1, by=.02), expand=c(0.05, 0)) +
    ...
    dev.off()
@
\input{figure/coverageByRegionRegimen.tex}
...
\paragraph{RSV season onset and offset are unchanged over time}

Our linear regression of median surseason onset and offset against surseason year (Figure \ref{fig:onsetOffsetByYear}) indicates (at 95\% confidence level) that season onset has not been drifting earlier (\Sexpr{formatDriftCI(onsetByYear, level)}), and neither has season offset (\Sexpr{formatDriftCI(offsetByYear, level)}). 
...
<<onsetOffsetByYear, results='hide', warning=FALSE>>=
    data = stateThresholdsByWindow %>% 
        ...
        rename(variable=variable_1, pctile=variable_2) %>%
        mutate(variable=factor(variable, levels=c("onset", "offset"))) %>%
        cast(year + variable ~ pctile)
        
    tikz("figure/onsetOffsetByYear.tex", width=plotWidth, height=plotHeight, pointsize=10)
    ggplot(data) +
        theme_light(base_size=10) +
        geom_errorbar(aes(ymin=lower, ymax=upper, x=year), width=.1) + 
        geom_point(aes(y=median, x=year)) +
        facet_wrap(~variable, ncol=1, scales="free_y", labeller=onsetOffsetLebeller) +
    ...
    dev.off()
@
\input{figure/onsetOffsetByYear.tex}
...
Compared to our regional analysis, the three regimens based on statewide recent-years analysis produced non-superior results in all regions except for the low-incidence counties, where they produced inferior results. (TODO: Figure.) TODO: specifics
...
Our model shows that fraction of RSV preventable by immunoprophylaxis in high-risk infants in Connecticut can be increased from that of the prophylaxis regimen currently recommended by the AAP. 
