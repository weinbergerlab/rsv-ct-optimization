...
\postauthor{
    ...
    \titlesection{What this study adds}{We present a method for optimizing the RSV prophylaxis schedule on a regional basis, and show that our method offers only a modest benefit when applied to the state of Connecticut, with no additional benefit when applied to counties within Connecticut.}
