...
install.packages(c("dplyr", "zipcode", "reshape", "mgcv", "ggplot2", "gridExtra", "devtools", "data.table"))
library(devtools)
...
for (c in levels(obsByCountyYear$county)) {
  for (y in rsvYears) {
    ...
    countyYearPred = data.frame(time=modelTime) %>% 
      ...
      cbind(countyYearModel %>% outbreak.predict.timeseries(modelTime, outbreak.calc.cum())) %>%
      rename(rsv.cum.fit.lower=lower, rsv.cum.fit.upper=upper, rsv.cum.fit=median) %>%
