...
<<seasonByRegion, results='hide'>>=
    tikz(sprintf("%s/seasonByRegion.tex", figuresDir), width=1.5*inlinePlotWidth, height=inlinePlotHeight, pointsize=10)
    ...
    ggplot(data) +
        ...
        labs(x=NULL) + 
        geom_rect(data=onsetOffsetMonthBoundaries, aes(xmin=min + 0.05, xmax=max - 0.05, ymin=0.6, ymax=0.8), color="grey70", fill="grey70") + 
        geom_text(data=onsetOffsetMonthBoundaries, aes(x=mid, label=label, y=0.7), color="white", size=2.2) + 
        theme(
...
<<onsetOffsetByYear, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        ...
        geom_blank(data=limits, aes(x=min)) + 
        geom_blank(data=limits, aes(x=max)) + 
        geom_rect(data=onsetOffsetMonthBoundaries, aes(xmin=min + 0.05, xmax=max - 0.05, ymin=1997.8, ymax=1998.2), color="grey70", fill="grey70") + 
