...
<<supplement_libraries, cache=FALSE>>=
@

<<supplement, cache=TRUE>>=
@
...
<<tollandPredMini, results='hide', warning=FALSE>>=
    ...
    ggplot(tollandPredMini) +
      ...
      geom_point(data=tollandObs, aes(x=time, y=rsv.cum.frac), size=0.5) +
      scale_x_continuous(breaks=monthBoundaries$mid, labels=epiWeekLabels, limits=range(c(monthBoundaries$min, monthBoundaries$max)), expand=c(0, 0)) + 
      theme(
        panel.grid.major.x=element_blank(),
        legend.title=element_blank(),
...
<<tollandOnsetMini, results='hide', warning=FALSE>>=
    ...
    ggplot(tollandPredMini) +
      ...
      geom_segment(aes(x=-Inf, y=seasonThreshold, xend=+Inf, yend=seasonThreshold), linetype="11", data=data.frame(), size=.375) + 
      geom_point(data=tollandObs, aes(x=time, y=rsv.cum.frac), size=0.5) +
      geom_point(data=tollandOnsetMini, aes(x=onset, y=seasonThreshold), size=0.75, shape=17) + 
...
Repeating this process many times (\Sexpr{simulations}, in our main analysis) then allows us to estimate the distribution of RSV season onset and therefore make interval estimates. In the case of Tolland county, the result of such season onset sampling is shown in Figure \ref{fig:tollandOnsetFull}.

<<tollandOnsetFull, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandOnsetFull.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)
    
    data = tollandPredFull
    splineData = data %>% filter(sim < tollandDisplayNsim)
...
\begin{figure}
\begin{center}
\input{\figuresDir/tollandOnsetFull}
\caption{Distribution of approximating $P$-splines ($N=\Sexpr{tollandDisplayNsim}$) and season onset ($N=\Sexpr{tollandNsimFull}$), for cumulative relative incidence of RSV-associated hospitalizations in Tolland county, Connecticut.}
\label{fig:tollandOnsetFull}
\end{center}
\end{figure}
...
<<tollandPredSingle, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandPredSingle.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)
      
    data = tollandPredSingle
    ...
    ggplot(data) +
      ...
      geom_point(data=tollandObs, aes(x=time, y=rsv.cum.frac, size=0.5), size=0.5) +
    	...
    	geom_text(data=thresholds, aes(x=max(monthBoundaries$max) - 1.5, y=mean(c(unprotected.start, unprotected.end)), label="$\\Delta$ = preventable fraction", angle=90, lineheight=0.5), color="white", size=1.75) + 
      scale_x_continuous(breaks=monthBoundaries$mid, labels=epiWeekLabels, limits=range(c(monthBoundaries$min, monthBoundaries$max)), expand=c(0, 0)) + 
      theme(
        panel.grid.major.x=element_blank(),
        legend.title=element_blank(),
...
<<tollandPreventableFractionFull, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandPreventableFractionFull.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)
    ...
    violinPlot <- ggplot(tollandFractionFull) +
      theme_light(base_size=plotTextBaseSize) +
      geom_violin(
...
\begin{subfigure}{.4\textwidth}
    \begin{center}
    \input{\figuresDir/tollandPredSingle}
    \caption{Preventable fraction derived from a randomly sampled $P$-spline approximating the RSV time series.}
    \label{fig:tollandPredMini}
    \end{center}
\end{subfigure}
\begin{subfigure}{.4\textwidth}
    \begin{center}
    \input{\figuresDir/tollandPreventableFractionFull}
    \caption{Distribution of approximating $P$-splines ($N=\Sexpr{tollandDisplayNsim}$) and preventable fractions ($N=\Sexpr{tollandNsimFull}$) of RSV under the AAP prophylaxis guidelines  in Tolland county, Connecticut.}
    \label{fig:tollandOnsetMini}
    \end{center}
\end{subfigure}
\end{center}
