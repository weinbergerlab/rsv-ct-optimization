...
knitr::read_chunk("../Paper/Supplement.R")
@
...
RSV seasonality and spatial variability have been established throughout the United States, based on National Respiratory and Enteric Virus Surveillance System data. In all states except for Florida, RSV shows an annual or biennial cycle with a stable seasonal onset between mid-November and early January and a stable seasonal offset between mid-March and late April.\cite{CentersforDiseaseControlandPreventionCDC:2011wt,CentersforDiseaseControlandPreventionCDC:2013wv} RSV season in Florida begins approximately two months earlier than in other states. \cite{CentersforDiseaseControlandPreventionCDC:2013wv}

The AAP acknowledges the potential significance of spatial and temporal variation in RSV incidence, but --- outside of Florida --- does not recommend local variance from nationwide prophylaxis guidelines.\cite{AmericanAcademyofPediatricsCommitteeonInfectiousDiseases:2014bj} 

Previous analysis of RSV-associated hospital admissions in Connecticu has shown regional variability in time of peak RSV incidence within Connecticut.\cite{Noveroske:y4fi3188} No prior research is available regarding year-to-year variation in timing of RSV season in Connecticut.
...
<<map, results='hide'>>=
    ...
    ggplot(map.df, aes(x=long, y=lat)) +
        ...
        scale_fill_manual(labels=legendLabels(c("$<$50", "$>$100")), values=c(lightBlue, darkBlue)) +
        geom_polygon(aes(fill=highRSV, group=group), color="white", size=.5) +
        geom_text(data=county.data, aes(label=name), size=2.2) +
        coord_map() + 
        labs(fill="Average RSV-associated hospitalizations per year") +
        theme(
            rect=element_blank(),
            line=element_blank(),
...
Our analysis thus far assumed that RSV seasons are similar year-to-year. To analyze the effects on prophylaxis regimens of year-to-year variation between RSV seasons, we chose to aggregate our data into (overlapping) periods of three consecutive years --- in order to maintain a higher level of statistical power, as well as to limit the impact of short-term variation on our results.

TODO This gave us \Sexpr{year(endDate) - year(startDate) -1} three-year periods, with the \Sexpr{formatYear(startDate + 2*365)} period covering surveillance years \Sexpr{formatYear(startDate)} through \Sexpr{formatYear(startDate + 2*365)}, and the \Sexpr{formatYear(endDate)} period covering \Sexpr{formatYear(endDate - 2*365)} through \Sexpr{formatYear(endDate)}.

We began our temporal analysis by estimating onset and offset for each three-year period, using the same penalized spline GAM as for the all-years analysis, producing --- as before --- statewide and regional estimates of \Sexpr{seasonThreshold * 100}\% onset and \Sexpr{(1 - seasonThreshold) * 100}\% offset.

To evaluate year-to-year variation in RSV season onset and offset we modeled median RSV three-year onset and offset against time using simple linear regression.
...
We then compared those regimens to the one recommended by the AAP and to the six spatially adjusted alternatives (with no calendrical rounding) discussed above.

As before, these alternative regimens involved five monthly doses of palivizumab giving 24 weeks of protection. 
...
<<seasonByRegion, results='hide'>>=
    ...
    plot = ggplot(data) +
        ...
        scale_y_continuous(minor_breaks=seq(1, 52), breaks=epiWeekBreaks, labels=rep("\\qquad\\qquad", length(epiWeekBreaks)), sec.axis=sec_axis(trans=~., breaks=seq(1, 52))) + 
        labs(y=NULL) + 
...
TODO Meanwhile, in comparison with the statewide RSV season onset and offset, the low-RSV region has RSV season with later onset at \Sexpr{regionOnset("lowIncidence")} weeks (\Sexpr{regionOnsetCI("all", level)} weeks); season offset shows no change from the statewide median (Figure \ref{fig:seasonByRegion}).
...
<<coverageByRegionAAPInsight, results='hide'>>=
    ...
    spanLabels = legendLabels(c("AAP-recommended prophylaxis", "RSV season"), leading=TRUE)
...
<<coverageByRegion, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        theme_light(base_size=plotTextBaseSize) +
        scale_y_continuous(labels=latexPercent, limits=c(0.9, 1), breaks=seq(0, 1, by=.02), expand=c(0.05, 0)) +
        scale_color_manual(name="Prophylaxis regimen", labels=legendLabels(stratLabels[c(seq(1, 6), 10)]), values=c(rep(c(darkBrown, darkOrange), each=3), darkPurple)) + 
        scale_shape_manual(name="Prophylaxis regimen", labels=legendLabels(stratLabels[c(seq(1, 6), 10)]), values=c(16, 15, 17, 16, 15, 17, 16)) + 
        geom_errorbar(
...
Based on all-years data, the six spatially adjusted prophylaxis regimens (with no rounding of prophylaxis dates), when compared to the AAP guidelines, yield non-inferior results in all regions, and superior results statewide (Figure \ref{fig:coverageByRegionRegimen}).

Statewide, the AAP guidelines offer protection to \Sexpr{regionStrategyCoverage("all", "aap", 0)} cases (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}). The alternative regimen based on all-years statewide onset yields the smallest increase, to \Sexpr{regionStrategyCoverage("all", "stateOnset", 0)} (\Sexpr{regionStrategyCoverageCI("all", "stateOnset", 0, level)}); other alternative regimens yield similar results.

Most of the increase comes from Hartford county, where the AAP guidelines protect \Sexpr{regionStrategyCoverage("Hartford", "aap", 0)} of cases (\Sexpr{regionStrategyCoverageCI("Hartford", "aap", 0, level)}), whereas the alternative regimen based on all-years regional offset generates the smallest increase, to \Sexpr{regionStrategyCoverage("Hartford", "countyOffset", 0)} (\Sexpr{regionStrategyCoverageCI("Hartford", "countyOnset", 0, level)}); other alternative regimens are comparable.

In every region, all alternative regimens based on all-years analysis are non-inferior to the AAP guidelines and non-superior to each other. 

\paragraph{Calendrical rounding rapidly erases gains over the AAP-recommended regimen}

<<calendarForRounding, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/calendarForRoundingRegimen.tex", figuresDir), width=pagePlotWidth, height=pagePlotHeight * 1.5, pointsize=10)
    
    roundingStrat = unprotectedByCounty %>% 
        filter(county %in% countiesForPlots, sliding==FALSE) %>%
        select(strat, rounding) %>%
        unique()
        
    dataAll = predByCounty %>%
        filter(county %in% countiesForPlots) %>%
        ...
        ungroup() %>%
        group_by(county, strat, rounding) %>%
        do(
            (function(df) {
                county = unique(df$county)
                rounding = as.numeric(unique(df$rounding))
                strat = as.character(unique(df$strat))
                strat = ppxFixedStrategies(county, rounding)[[strat]]
                stratActive = strat(df$time)
                df %>%
                    mutate(stratActive=stratActive)
            })(.)
        ) %>%
        ungroup() %>%
        filter(stratActive > 0) %>%
        group_by(county, strat, rounding) %>%
    ...
    aap = dataAll %>%
        filter(strat != "aap") %>%
        group_by(strat, rounding) %>%
        do(
            aapStartEnd
        ) %>%
        ungroup() %>%
        mutate(y=ifelse(grepl("state", strat), "all", "lowIncidence")) %>%
        mutate(y=factor(y, levels(dataAll$county)))
        
    data = dataAll %>% 
        filter(strat != "aap") %>%
        filter(xor(grepl("county", strat), county == "all"))
    
    ggplot(data) +
        theme_light(base_size=plotTextBaseSize) +
    	geom_rect(aes(xmin=start, xmax=end, ymin=as.numeric(county)-0.25, ymax=as.numeric(county)+0.25), size=.375) + 
    	geom_rect(data=aap, aes(xmin=start, xmax=end, ymin=as.numeric(y)+0.75, ymax=as.numeric(y)+1.25), size=.375) + 
        scale_y_continuous(breaks=as.numeric(countiesForPlots), labels=countyLabels[as.character(countiesForPlots)]) + 
        scale_x_continuous(breaks=monthBoundaries$mid, labels=epiWeekLabels, limits=range(c(monthBoundaries$min, monthBoundaries$max)), expand=c(0, 0)) + 
#        scale_color_manual(labels=spanLabels, values=twoToneAlt) +
#        scale_fill_manual(labels=spanLabels, values=twoToneAlt)# +
#        guides(color=guide_legend(keywidth=3, keyheight=.4)) + 
        facet_wrap(~ rounding + strat, nrow=8, labeller=labeller(rounding=roundingLabels, strat=stratLabels), scales="free_y") #+
#        labs(x=NULL, y="Cumulative indicence fraction") + 
#        theme(
#            panel.grid.major.x=element_blank(),
#            legend.title=element_blank(),
...
<<coverageByRounding, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        ...
        scale_color_manual(
          ...
          labels=legendLabels(c(as.vector(stratLabels[10]), "_", "__", as.vector(stratLabels[1:6]))),
          values=c(
...
After we applied weekly, biweekly, and monthly rounding to start and end dates of our six alternative immunoprophylaxis regimens, we found that longer rounding intervals diminished the benefit of spatial analysis (Figure \ref{fig:coverageByRoundingRegimen}).

With weekly rounding, our regimens were non-inferior to the AAP recommendation, and produced an increase in preventable fraction from AAP's \Sexpr{regionStrategyCoverage("all", "aap", 0)} (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}) to \Sexpr{bestCaseCoverage(1)} (\Sexpr{bestCaseCoverageCI(1, level)}) statewide. 

Biweekly rounding yielded regimens similarly non-inferior to the AAP recommendation, with an increase in preventable fraction to \Sexpr{bestCaseCoverage(2)} (\Sexpr{bestCaseCoverageCI(2, level)}) statewide. 

However, with monthly rounding, the gains almost completely vanished, with preventable fraction not exceeding \Sexpr{bestCaseCoverage(4)} (\Sexpr{bestCaseCoverageCI(4, level)}). Furthermore, all our regimens with monthly rounding were non-superior to the AAP-recommended regimen in all regions.

\begin{figure}
\begin{center}
\input{\figuresDir/coverageByRoundingRegimen.tex}
\caption[The effect of calendrical rounding of start and end of spatially adjusted RSV prophylaxis regimens on their performance]{The effect of calendrical rounding of start and end of spatially adjusted RSV prophylaxis regimens on their performance, showing that longer rounding intervals diminish the benefits of alternative regimens over the AAP-recommended regimen. Preventable fraction: the fraction of all RSV hospitalizations occurring while the prophylaxis regimen offers protection. Hollow plot markers ($\Circle$, $\square$, and $\triangle$) denote a prophylaxis strategy which, as a result of rounding, is identical to another, and therefore does not provide additional insights.}
...
Our linear regression of median RSV season onset and offset over three-year periods indicates that season onset has slowly been drifting earlier (Figure \ref{fig:onsetOffsetByYear}), at a rate of \Sexpr{formatDrift(onsetByYear)} (\Sexpr{formatDriftCI(onsetByYear, level)}). It is unclear whether this is a part of a larger pattern, such as a cycle spanning decades. 
...
<<coverageByRegionWithSliding, results='hide', warning=FALSE>>=
    ...
    ggplot(data) +
        theme_light(base_size=plotTextBaseSize) +
        scale_y_continuous(labels=latexPercent, limits=c(0.9, 1), breaks=seq(0, 1, by=.02), expand=c(0.05, 0)) +
        scale_color_manual(name="Prophylaxis regimen", labels=legendLabels(stratLabels[c(seq(1, 3), seq(7, 10))]), values=c(rep(c(darkBrown, darkGreen), each=3), darkPurple)) + 
        scale_shape_manual(name="Prophylaxis regimen", labels=legendLabels(stratLabels[c(seq(1, 3), seq(7, 10))]), values=c(16, 15, 17, 16, 15, 17, 16)) + 
        geom_errorbar(
            aes(ymin=frac.lower, ymax=frac.upper, x=county, color=strat, group=strat), 
            width=.3, 
            position=position_dodge(.5)
...
Compared to our regional all-years analysis, the three regimens based on statewide recent-years analysis produced non-superior results everywhere, and inferior results in the low-RSV counties (Figure \ref{fig:coverageByRegionRegimenWithSliding}).
...
% TODO figure showing prophylaxis regimens on a calendar?
...
Increasing complexity of practice guidelines carries with it increased cost of implementation, such as training of clinicians and other healthcare workers. It is therefore not apparent from our research alone that the potential benefits would outweigh the costs. 
...
Similarly, we assumed the protection by palivizumab is constant at 100\% throughout the protection period; however, not only is breakthrough RSV illness known to occur in patients on palivizumab, but it is an indication for discontinuing the prophylaxis regimen. 

In areas of Connecticut associated with a relatively high amount of interstate travel (such as urban centers near the state lines), our analysis --- by only considering residents of Connecticut --- likely underestimates RSV burden.

Finally, towns that see substantial inter-county travel (for example, employment hubs near county lines) likely bias our regional analysis.
...
A simple application of this is our estimation of RSV season onset. Consider, for example, RSV-associated hospitalizations in Tolland County, Connecticut in Figure \ref{fig:tollandCases}. (This is a low-incidence county, which makes it a poor candidate for our main analysis, but its large confidence intervals make it easier to visualize our methods.)

<<tollandCases, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandCases.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)

    ggplot(tollandObs) +
        theme_light(base_size=plotTextBaseSize) +
        geom_point(aes(x=time, y=rsv.cum.frac)) +
        scale_x_continuous(breaks=monthBoundaries$mid, labels=epiWeekLabels, limits=range(c(monthBoundaries$min, monthBoundaries$max)), expand=c(0, 0)) + 
        theme(
            panel.grid.major.x=element_blank(),
            legend.title=element_blank(),
...
\begin{figure}
\begin{center}
\input{\figuresDir/tollandCases.tex}
\caption{Cumulative relative incidence of RSV-associated hospitalizations in Tolland county, Connecticut, aggregated across years \Sexpr{formatYear(startDate)}-\Sexpr{formatYear(endDate)}
\label{fig:tollandCases}
\end{center}
\end{figure}

<<tollandPredMini, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandPredMini.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)

    ggplot(tollandPredMini) +
      theme_light(base_size=plotTextBaseSize) +
...
\begin{figure}
\begin{center}
\input{\figuresDir/tollandPredMini}
...
\label{fig:tollandPredMini}
\end{center}
\end{figure}

<<tollandOnsetMini, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandOnsetMini.tex", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)
    ...
    ggplot(tollandPredMini) +
      ...
      geom_point(data=tollandOnsetMini, aes(x=onset, y=seasonThreshold)) + 
      geom_segment(data=tollandOnsetMini, aes(x=onset, xend=onset, y=seasonThreshold, yend=0), linetype="11") + 
      theme(
        panel.grid.major.x=element_blank(),
        legend.title=element_blank(),
        axis.title.y=element_blank(),
...
\begin{figure}
\begin{center}
\input{\figuresDir/tollandOnsetMini}
\caption{Five randomly sampled approximations of cumulative relative incidence of RSV-associated hospitalizations in Tolland county, Connecticut, with their corresponding season onsets.}
\label{fig:tollandOnsetMini}
\end{center}
\end{figure}
...
We randomly sample each coefficient from its distribution, thereby obtaining a specific spline that smoothly approximates our data. Five such randomly sampled splines are shown in Figure \ref{fig:tollandPredMini}. We then calculate RSV season onset for each sampled spline (Figure \ref{fig:tollandOnsetMini}).

Repeating this process many times (\Sexpr{simulations}, in our main analysis) then allows us to estimate the distribution of RSV season onset and therefore make interval estimates. In the case of Tolland county, the result of similar season onset sampling is shown in Figure \ref{fig:tollandPredFull}.

<<tollandOnsetFull, results='hide', warning=FALSE>>=
    tikz(sprintf("%s/tollandOnsetFull", figuresDir), width=pagePlotWidth * 0.4, height=pagePlotHeight, pointsize=10)

    ggplot(tollandPredFull) +
      theme_light(base_size=plotTextBaseSize) +
      geom_ribbon(aes(x=time, ymin=min, ymax=max), fill="gray", na.rm=FALSE) +
      geom_segment(aes(x=-Inf, y=seasonThreshold, xend=+Inf, yend=seasonThreshold), linetype="11", data=data.frame(), size=.375) + 
      geom_violinh(
        data=tollandOnsetFull,
      ...
      scale_y_continuous() +
      coord_cartesian(xlim=c(zoomedStartWeekFull, zoomedEndWeekFull), ylim=c(0, 4*seasonThreshold)) +
      theme(
        panel.grid.major.x=element_blank(),
        legend.title=element_blank(),
        axis.title.y=element_blank(),
...
\begin{figure}
\begin{center}
\input{\figuresDir/tollandPredFull}
\caption{Distribution of approximating $P$-splines and the calculated season onset with $N=\Sexpr{simulations}$ samples, for cumulative relative incidence of RSV-associated hospitalizations in Tolland county, Connecticut.}
\label{fig:tollandPredFull}
\end{center}
\end{figure}
...
More interestingly, the same approach can be used to estimate time-series outcomes. We use this in our computation of preventable cases of RSV. We begin by defining a preventable case of RSV to be a case of RSV that occurs while prophylaxis (under a specific set of prophylaxis guidelines) offers protection; that is, a case that would have been prevented by administration of prophylaxis under those guidelines, if the prophylaxis were 100\% effective. As before, we obtain a $P$-spline approximation to our data.

We then sample $P$-spline coefficients from their distributions to obtain a specific approximation to our data, and compute the corresponding time series of preventable RSV cases. Repeating the coefficient-sampling process yields time series of preventable RSV cases sampled from their (again unknown) distribution.

This process can also be generalized to any time-series outcome of an infectious disease outbreak or season.

Furthermore, the two approaches can be combined to calculate complex non-linear outcomes. We use this to estimate the fraction of preventable RSV cases in an RSV season, defined as the number of preventable cases divided by the total number of cases. The preventable fraction is a non-linear function of the RSV time series; as a result, interval estimation of the preventable fraction cannot be obtained by point-wise estimation of the RSV time, followed by applying a function to point-wise estimates. Instead, estimates of the entire time series must be sampled, and the preventable fraction then calculated for each sampled time series. 

Here we follow a similar process, wherein we sample coefficients from their distributions, use the resultant $P$-spline to calculate the all-case time series as well as the preventable-case time series, from which we then calculate the preventable fraction; the end result is that we sample the preventable fraction from its (unknown) distribution. 
