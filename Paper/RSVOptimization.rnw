...
\abstractsection{Results}Using preventable fraction to assess immunoprophylaxis regimens, we found that regimens adjusted for county-level variation in RSV seasons are superior to the AAP-recommended regimen. Our best alternative to the AAP guidelines was based on the county-level RSV season midpoint; in comparison to the AAP recommendation, whose preventable fraction is \Sexpr{regionStrategyCoverage("all", "aap", 0)} (\Sexpr{regionStrategyCoverageCI("all", "aap", 0, level)}), that alternative yielded improvement to \Sexpr{regionStrategyCoverage("all", "countyMiddle", 1)} (\Sexpr{regionStrategyCoverageCI("all", "countyMiddle", 1, level)}). 
