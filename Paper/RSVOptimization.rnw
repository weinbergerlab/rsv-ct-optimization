...
knitr::read_chunk("../Paper/RSVOptimization.R")
@
...
<<coverageByRounding, results='hide', warning=FALSE>>=
    ...
    # We are going to dim some data points, because sometimes different strategies under different rounding end up producing the same prophylaxis schedule, and there is no point in showing all of them equally on the plot. To do this, we consider the left-to-right order in which strategies will appear in the plot, and dim any strategy whose schedule is the same as some strategy that appears on its left
    data = unprotectedByCounty %>% 
        filter(county == "all", sliding==FALSE) %>%
        arrange(rounding, strat)

    data$startIdx = match(data$stratStart, data$stratStart)
    ...
    ggplot(data) +
        ...
        geom_errorbar(
            aes(ymin=frac.lower, ymax=frac.upper, x=rounding, color=strat, group=strat, alpha=isRepeat), 
            width=.25, 
            position=position_dodge(.5)
        ) + 
        geom_point(aes(y=frac.median, x=rounding, color=strat, shape=strat, alpha=isRepeat), position=position_dodge(.5), size=.75) +
...
A simple example of this is our estimation of RSV season onset. We begin the process by defining season onset to occur when cumulative incidence rises above \Sexpr{seasonThreshold * 100}\% of total cases, and obtaining a $P$-spline approximation to our data (using the \texttt{mgcv} R package). This approximation is characterized by a matrix of coefficients, each of which is normally distributed with a known mean and variance. 

We randomly sample each coefficient from its distribution, thereby obtaining a specific spline that smoothly approximates our data. We calculate RSV season onset for this specific spline, and then repeat the coefficient sampling process, thereby iteratively sampling RSV season onset from its (unknown) distribution.
